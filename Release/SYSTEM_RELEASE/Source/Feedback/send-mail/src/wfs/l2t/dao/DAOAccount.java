...
public class DAOAccount extends DAO {
	...
	public List<dtoAccount> getAllAccount(String time) {
		dtoAccount account = null;
		List<dtoAccount> listAccount = new ArrayList<dtoAccount>();
		String sql = "SELECT `AccountId`, `UserName`, `Password` , `Email`, `TimeReceiveEmail`, `NumberReceiveEmail` FROM `account` where `TimeReceiveEmail`='"+time+"'";
		if (this.connection.connect()) {
			ResultSet rs = this.connection.read(sql);
			try {
				while (rs.next()) {
