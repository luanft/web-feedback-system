...
public class ControllerAccount extends HttpServlet
{
		...
		if (ServletFileUpload.isMultipartContent(request))
		{
			...
			for (FileItem i : files)
			{
				if (!i.isFormField())
				{
					...
					i.write(new File(path + "/view/resource/image/avatar/" + list + "."
							+ FilenameUtils.getExtension(i.getName()).toUpperCase()));

					ModelAccount account = new ModelAccount();
					account.changeAvatar(this.loginUtility.getLoggedUserId(), "/view/resource/image/avatar/" + list
							+ "." + FilenameUtils.getExtension(i.getName()).toUpperCase());
				}
			}
		}
	...
	private void changePassword(HttpServletRequest request, HttpServletResponse response)
	{
		...
		{// nofity error: password not match
			errorMessage = "Mật khẩu cũ không khớp!";
		}

	}

	public Boolean changeUserName(HttpServletRequest request, HttpServletResponse response)
	{
		String newUserName = request.getParameter("fcn-new-user-name");
		ModelAccount account = new ModelAccount();
		return account.changeUserName(this.loginUtility.getLoggedUserId(), newUserName);
	}
			...
			IOException
	{
		...
		} catch (Exception e)
		{
			...
			request.setAttribute("user", loginUtility.getLoggedUserId());
			request.setAttribute("error_message", "Lỗi xảy ra khi thay thế avatar");
			request.getRequestDispatcher("view/account-manager.jsp").include(request, response);
			e.printStackTrace();
		}
	}
